  ...
  return (
    <div style={{ padding: '24px 28px' }}>
      ...
      <div style={{
        ...
        {blocked ? <AlertOctagon size={18} color="#dc2626" /> : <CheckCircle2 size={18} color="#16a34a" />}
        <div>
          <p style={{ fontSize: 13.5, fontWeight: 700, color: blocked ? '#b91c1c' : '#15803d' }}>
            {blocked
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 14 }}>
        ...
        />
        <ExportCard
          icon={<FileJson size={20} color="#0891b2" />}
      ...
      <div style={{
        ...
        <strong style={{ color: '#475569' }}>Note:</strong> These exports are draft artifacts. They are not certified or
...
function ExportCard({ icon, title, desc, buttonLabel, onClick, accent = '#2563eb' }: {
  icon: React.ReactNode; title: string; desc: string; buttonLabel: string;
  onClick: () => void; accent?: string;
}) {
  return (
    <div style={{
      border: '1px solid #e2e8f0', borderRadius: 14, padding: '18px 18px 16px',
      ...
      <div style={{ flex: 1 }}>
        <h3 style={{ fontSize: 14.5, fontWeight: 700, color: '#1e293b', marginBottom: 5 }}>{title}</h3>
        <p style={{ fontSize: 12.5, color: '#64748b', lineHeight: 1.5 }}>{desc}</p>
      </div>
