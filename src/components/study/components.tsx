...
import {
  AlertTriangle, Check, ClipboardCheck, Copy, CopyPlus, Folder, FolderTree,
...
export function VisitPicker({ visits, activeVisitId, onSelect, onReorder }: {
  ...
  // If the active visit moves to a different arm from elsewhere (e.g. it's
  // deleted and selection falls back to a visit in another arm), follow it.
  // Adjusted during render (not an effect) — the standard React pattern for
  // state that must react to a prop change without an extra render+flash.
  const [lastActiveId, setLastActiveId] = useState(activeVisitId);
  if (activeVisitId !== lastActiveId) {
    setLastActiveId(activeVisitId);
    const nextArm = active?.arm ?? arms[0] ?? 'Study Visit';
    if (nextArm !== armTab) setArmTab(nextArm);
  }
  ...
  return (
    <div style={{ width: '100%' }}>
      {/* Arm tabs — always visible, not a dropdown */}
        ...
        {arms.map(arm => {
          ...
          return (
            <button key={arm} onClick={() => setArmTab(arm)} style={{
              ...
              padding: '7px 12px', border: 'none', borderRadius: '8px 8px 0 0', cursor: 'pointer',
              background: armActive ? '#fff' : 'transparent',
              ...
              <span style={{
                ...
                background: armActive ? '#FDF1F1' : '#F1EFEA', color: armActive ? '#973C38' : '#A29C90',
              }}>{count}</span>
            </button>
          );
        })}
      ...
      {/* Visit/folder schedule for the selected arm */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 2px 2px', overflowX: 'auto' }}>
        <SortableList ids={ids} onReorder={(from, to) => onReorder(ids[from], ids[to])} strategy="horizontal">
          {armVisits.map(v => (
            <SortableRow key={v.id} id={v.id}>
              {({ setNodeRef, style, handleProps }) => {
                const isActive = v.id === activeVisitId;
                return (
                  <div ref={setNodeRef} style={{ ...style, flexShrink: 0, display: 'flex', alignItems: 'center' }}>
                    <button onClick={() => onSelect(v.id)} style={{
                      display: 'flex', alignItems: 'center', gap: 6, padding: '8px 12px', borderRadius: 9,
                      border: `1.5px solid ${isActive ? '#BE4A46' : '#E6E3DC'}`,
                      background: isActive ? '#FDF1F1' : '#fff', cursor: 'pointer', whiteSpace: 'nowrap',
                    }}>
                      <span {...handleProps} title="Drag to reorder" style={{ display: 'inline-flex', color: isActive ? '#F1CFCE' : '#DCD8CF', cursor: 'grab', touchAction: 'none' }} onClick={(e) => e.stopPropagation()}>
                        <GripVertical size={13} />
                      </span>
                      <span style={{ fontSize: 13.5, fontWeight: 600, color: isActive ? '#BE4A46' : '#17181A' }}>{label(v)}</span>
                    </button>
                  </div>
                );
              }}
        ...
        {armVisits.length === 0 && (
          <p style={{ fontSize: 12.5, color: '#A29C90', padding: '4px 6px' }}>No folders in this arm.</p>
        )}
      </div>
    </div>
  );
