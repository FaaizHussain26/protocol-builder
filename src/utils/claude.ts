...
4. EXTRACT EVERY STUDY VISIT from the SOA columns. Read the protocol THOROUGHLY and STRICTLY: every single column in the Schedule of Activities is a visit and MUST appear in the output — do not collapse, summarize, sample, or skip intermediate timepoints. If the SOA has columns for Screening, Baseline/Day 1, Day 8, Week 2, Week 4, Week 8, Week 12, End of Treatment, End of Study, and Follow-Up, then output ALL of them, in order. Capture each visit's sequence, timing, and window. A typical phase II/III protocol has many visits (often 8-15+); returning only a few means you have missed columns — go back to the SOA and read every column. Continuous/unscheduled collections (Adverse Events, Concomitant Medications) are kind "log".
